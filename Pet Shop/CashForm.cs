    ...
    public partial class CashForm : Form
    {
        ...
        removeitem:
            if (colName == "Delete")
            {
                ...
                {
                    Databasecon.executeQuery("DELETE FROM Cashtable WHERE cashid LIKE '" + dgvCash.Rows[e.RowIndex].Cells[1].Value.ToString() + "'");
            ...
            else if (colName == "Increase")
            {
                ...
                {
                    Databasecon.executeQuery("UPDATE Cashtable SET qty = qty + " + 1 + " WHERE cashid LIKE '" + dgvCash.Rows[e.RowIndex].Cells[1].Value.ToString() + "'");
                }
                else
                {
        ...
        private void buttonCash_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Are you sure you want to cash this product?", "Cashing", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                Databasecon.executeQuery("UPDATE Producttable SET pqty= pqty - " + int.Parse(dgvCash.Rows[i].Cells[4].Value.ToString()) + " WHERE pcode LIKE " + dgvCash.Rows[i].Cells[2].Value.ToString() + "");
                }
                dgvCash.Rows.Clear();
            }
        }
...
        public void getTransno()
        {
            try
            {
                ...
                cn.Open();
                cm = new SqlCommand("SELECT TOP 1 transno FROM Cashtable WHERE transno LIKE '" + sdate + "%' ORDER BY cashid DESC", cn);
                dr = cm.ExecuteReader();
                dr.Read();
        ...
        public void loadCash()
        {
            try
            {
                ...
                double total = 0;
                dgvCash.Rows.Clear();
                cm = new SqlCommand("SELECT cashid,pcode,pname,qty,price,total,c.name,cashier FROM Cashtable as cash LEFT JOIN Customertable c ON cash.cid = c.id WHERE transno LIKE " + lblTransno.Text + "", cn);
                cn.Open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
