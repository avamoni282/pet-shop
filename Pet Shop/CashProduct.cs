...
namespace Pet_Shop
{
    public partial class CashProduct : Form
    {
              ...
              bool chkbox = Convert.ToBoolean(dr.Cells["Select"].Value);
                ...
                { 
...

                    {
                        ...
                        cm = new SqlCommand("INSERT INTO Cashtable(transno, pcode, pname, qty, price, cashier) VALUES (@transno, @pcode, @pname, @qty, @price, @cashier)", cn);
        ...
        public void LoadProduct()
        {
            ...
            cm = new SqlCommand("SELECT pcode, pname, ptype, pcategory, pprice FROM Producttable WHERE CONCAT(pname,ptype,pcategory) LIKE '%" + txtSearch.Text + "%' AND pqty > " + 0 + "", cn);
            cn.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
